...
def main():
   ...
   set_frame()
   path = os.path.join(dirname(dirname(dirname(__file__))), "config", "text.json")
   text = read_json(path)
   st.title(text["title"]["text"]) 

   tab0, tab1, tab2 = st.tabs(["Inputs", text["tabs"]["tab1"]["title"], text["tabs"]["tab2"]["title"]])
      ...
      def current_infra():
         if (Run | RunInfra): 
            ...
            try:
               ...
               display_results_current_infra_optimal(df1)
      ...
      def ideal_infra():
         if RunInfra:
            ...
            with st.spinner("Crunching Some Numbers"):
               ...
               chart_data1, chart_data2 = optimize(cluster_perc, MemRequired, NormalizedDataLoad, slices, presliced_flag, VMem, VCores, Nodes, CurrentInfraCalc)
