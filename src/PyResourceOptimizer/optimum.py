...
def optimize(CalcMode):
    ...
    mask4 = (
        df.MaxExecPerNode * (df.ExecutorMemory + df.MemoryOverhead)
        < df.available_memory_per_node
    )
    df = df[mask3][mask4]

    df["TotalMemUsed%"] = (df["TotalMemUsed"] / df.available_memory) * 100
    df["BalancedOptimum"] = df.Exec + 3*df.Cores_y
    utilities.update_max_memory()
    end = dt.now()
...
def get_runtime_vs_cost_line_chart_data(full_df):
    ...
    full_df_optimum["tooltip"] = (
        full_df_optimum["node_count"].astype("str")
        + " * "
        + full_df_optimum["name"]
        + " --> "
        + full_df_optimum["MaxSerialSlices"].astype("str")
        + "x Runtime"
    )
